...
import static net.minecraft.world.effect.MobEffects.CONDUIT_POWER;
...
public interface BrightnessSampler {
	BrightnessSampler LEGACY = (level, player, position) -> {
		...
		if (player.isOnFire() || player.hasEffect(NIGHT_VISION)) {
			return 1.0D;
		}

		final BlockPos blockPos = new BlockPos(position);

		if (player.hasEffect(CONDUIT_POWER) && level.getFluidState(blockPos).is(WATER)) {
			return 1.0D;
		}
